...
### this code is my second attempt to predict the passengers that died onboard the Titanic, using Kaggle's titanic competition data
...
plt.figure(figsize=(15,8))
ax = sns.barplot(x=df_train['Embarked'], y=df_train['Survived'], hue=df_train['Sex'], data=df_train)
plt.title('Bar Graph of Port of Departure and Survival with Sex Differentiation')
plt.show()
plt.close()

### we can see from this bar graph that passengers embarking from port C had the highest survival rates for both men and women
### but again women had a much higher survival rate across all ports than men
